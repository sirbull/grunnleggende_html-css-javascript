...
import { axeViolations, currentLesson, lessonTitle, showResult } from './helpers.js';
...
test('four new beginner cases have working interactions', async ({page}) => {
  ...
  await expect(lessonTitle(page)).toHaveText('En teller med pluss og minus');demo=await showResult(page);
  ...
  await page.goto('#/combined/basic/project-color');await expect(lessonTitle(page)).toHaveText('En fargevelger for et kort');demo=await showResult(page);
  await demo.getByRole('combobox').selectOption('hav');await expect(demo.locator('#kort')).toHaveClass('kort hav');
  await page.goto('#/combined/basic/project-tips');await expect(lessonTitle(page)).toHaveText('Et tips om gangen');demo=await showResult(page);
  await demo.getByRole('button',{name:'Forrige tips'}).click();await expect(demo.locator('#posisjon')).toHaveText('Tips 3 av 3');
  await page.goto('#/combined/basic/project-gallery');await expect(lessonTitle(page)).toHaveText('En liten bildevelger');demo=await showResult(page);
...
test('four new intermediate cases validate and update their models',async({page})=>{
  let demo;
  await page.goto('#/combined/intermediate/project-budget');await expect(lessonTitle(page)).toHaveText('En enkel budsjettkalkulator');demo=await showResult(page);
  await demo.getByRole('button',{name:'Beregn'}).click();await expect(demo.locator('#resultat')).toContainText('900');
  await page.goto('#/combined/intermediate/project-menu');await expect(lessonTitle(page)).toHaveText('En tilgjengelig mobilmeny');demo=await showResult(page);
  ...
  await page.goto('#/combined/intermediate/project-faq');await expect(lessonTitle(page)).toHaveText('Spørsmål og svar med én åpen om gangen');demo=await showResult(page);
  ...
  await page.goto('#/combined/intermediate/project-sort');await expect(lessonTitle(page)).toHaveText('Sorter et lite bibliotek');demo=await showResult(page);
...
test('four new advanced cases handle state, filters, URLs and modal editing',async({page})=>{
  let demo;
  await page.goto('#/combined/advanced/project-cart');await expect(lessonTitle(page)).toHaveText('En handlekurv uten betaling');demo=await showResult(page);
  ...
  await page.goto('#/combined/advanced/project-pagination');await expect(lessonTitle(page)).toHaveText('Søk, filter og sideinndeling');demo=await showResult(page);
  ...
  await page.goto('#/combined/advanced/project-bookmarks');await expect(lessonTitle(page)).toHaveText('En lokal leseliste');demo=await showResult(page);
  ...
  await page.goto('#/combined/advanced/project-edit-dialog');await expect(lessonTitle(page)).toHaveText('Rediger et kort i en modal dialog');demo=await showResult(page);
